...
sudo apt purge -y nginx nginx-common nginx-full \
  ...
  libnginx-mod-stream libmaxminddb0 || true
...
server {
    ...
    # Modern SSL configuration for Ubuntu 24.04
...
# Ensure sites-enabled directory exists (Ubuntu 24.04 structure)
sudo mkdir -p /etc/nginx/sites-enabled

echo "=== Creating test page..."

sudo mkdir -p /var/www/html
echo "<h1>NGINX on WSL Ubuntu 24.04 is working!</h1>" | sudo tee /var/www/html/index.html >/dev/null

echo "=== Testing and restarting NGINX..."
sudo nginx -t && sudo systemctl restart nginx

echo "=== NGINX installation complete!"
echo "Visit: https://localhost (from Windows browser)"
