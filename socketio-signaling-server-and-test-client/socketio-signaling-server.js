...
io.on('connection', (socket) => {
  ...
  socket.on('file-init', ({ fileName, totalChunks, to }) => {
    socket.fileTransfer = { fileName, totalChunks, chunks: [], to }
    io.to(to).emit('file-meta', { fileName, totalChunks, from: socket.id })
  })

  socket.on('file-chunk', ({ chunk, index }) => {
    if (!socket.fileTransfer) return
    socket.fileTransfer.chunks[index] = chunk
    io.to(socket.fileTransfer.to).emit('file-chunk', { chunk, index })
  })

  socket.on('file-complete', () => {
    if (!socket.fileTransfer) return
    io.to(socket.fileTransfer.to).emit('file-complete', { fileName: socket.fileTransfer.fileName })
    delete socket.fileTransfer
  })
})
