...
const std::string RTSP_URL = "rtsp://192.168.29.251:5543/live/channel0";
...
std::string generate_peer_id() {
    ...
    return "rtsp_cam_" + std::to_string(distr(gen));
}
...
// --- RTSP Stream Capture Loop ---
void rtsp_video_loop() {
    // Force FFmpeg to use TCP for RTSP transport to prevent UDP packet loss/artifacts
    ...
    auto frame_duration = std::chrono::milliseconds(33); // ~30 FPS throttle
    ...
    while (running_capture) {
        if (!video_capture.isOpened()) {
            ...
            // Set network connection timeout (5 seconds)
            video_capture.open(RTSP_URL, cv::CAP_FFMPEG);
            video_capture.set(cv::CAP_PROP_OPEN_TIMEOUT_MSEC, 5000);

            if (!video_capture.isOpened()) {
                std::cerr << "❌ Failed to connect to RTSP stream. Retrying in 3 seconds..." << std::endl;
                std::this_thread::sleep_for(std::chrono::seconds(3));
                continue;
            }
            std::cout << "✅ Connected to RTSP stream successfully!" << std::endl;
        }

        auto start_time = std::chrono::steady_clock::now();

        if (!video_capture.read(frame) || frame.empty()) {
            std::cerr << "⚠️ Frame drop or stream disconnect detected. Re-establishing RTSP..." << std::endl;
            video_capture.release();
            std::this_thread::sleep_for(std::chrono::milliseconds(500));
            continue;
        }

        // Resize frame if target resolution differs
...
int main() {
    ...
    std::thread capture_thread(rtsp_video_loop);
                ...
                local_pc->onDataChannel([&, this_session](std::shared_ptr<rtc::DataChannel> dc) {
                    if (dc->label() == "video-stream") {
                        ...
                        video_channel->onOpen([&, this_session]() {
                            ...
                            if (this_session != current_session_id) return;
                            std::cout << "🚀 Video Data Channel Connected. Streaming RTSP Camera [Session #" << this_session << "]." << std::endl;
    ...
    std::cout << "============================================" << std::endl;
    std::cout << "🎥 RTSP WEBRTC TRANSCEIVER ONLINE" << std::endl;
    std::cout << "SOURCE: " << RTSP_URL << std::endl;
